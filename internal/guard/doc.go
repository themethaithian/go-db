...
// the two shapes go-db accepts — db.<collection>.<verb>(<args>) and
// db.<verb>(<args>), and not JavaScript, which could not be classified without
// being evaluated — and then judges the operation against a closed allowlist of
// reads, one list per shape. aggregate is the one operation whose argument
// decides it: its pipeline is a read only when every stage in it, and in every
// subpipeline its stages carry, is on a closed list of stages that do not write.
// $out and $merge do; a stage nobody has listed is refused for being unlisted
// rather than assumed harmless. The database-level list holds one entry,
// getCollectionNames, which is the catalogue read the Database tree browses a
// MongoDB Profile with — dropDatabase, createCollection, runCommand,
// adminCommand and getSiblingDB are mutations, and say why. The two lists are
// separate on purpose: a name proven at one level proves nothing at the other.
// Anything that will not parse — SQL, a Redis command, a shell helper, a buffer
